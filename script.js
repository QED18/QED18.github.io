...
const buildVersion = body.dataset.buildVersion || "dev";
...
function syncVersionTag() {
    if (versionTag) {
        versionTag.textContent = `Build ${buildVersion}`;
    }
}
...
function applyReducedMotionPreference() {
    ...
    if (reducedMotionQuery.matches) {
        ...
        setIdleNowPlayingBars();
        setStatus(
            "Reduced motion is enabled. Showing a still frame from the background video.",
            "info"
        );
        return;
    }
...
function handleBlockedInteraction(event) {
    ...
    setStatus("Inspect shortcuts are disabled on this page.", "warning");
}

function handleKeydown(event) {
    const shouldBlock = blockedKeyCombos.some((matcher) => matcher(event));

    if (shouldBlock) {
        handleBlockedInteraction(event);
    }
}
